...
export default function CosmicBackground() {
  ...
  useEffect(() => {
    ...
    const auraMat = new THREE.ShaderMaterial({
        uniforms: { uTime: { value: 0 }, uIntensity: { value: 1.0 } } as any,
        vertexShader: `
  ...
  return (
    <div className="fixed inset-0 w-full h-full -z-20 overflow-hidden bg-[#010103]">
      <div ref={mountRef} className="absolute inset-0 w-full h-full" />
      <div 
        id="vignette"
        className="fixed inset-0 pointer-events-none z-5" 
        style={{ background: "radial-gradient(circle, transparent 50%, black 150%)" }} 
      />
      <div 
        id="overlay" 
        className="fixed inset-0 pointer-events-none flex flex-col justify-between p-10 z-10"
